                                                            ...
                                                            size_t size)
{
    ...
    ucx_datatype->op_param.isend = ucx_datatype->op_param.send;
    ucx_datatype->op_param.irecv = ucx_datatype->op_param.recv;
    ucx_datatype->op_param.isend.op_attr_mask |= opal_common_ucx.op_attr_nonblocking;
    ucx_datatype->op_param.irecv.op_attr_mask |= opal_common_ucx.op_attr_nonblocking;
...
ucp_datatype_t mca_common_ucx_init_datatype(ompi_datatype_t *datatype)
{
    ...
    if (datatype->pml_data != COMMON_UCX_DATATYPE_INVALID) {
...
out:
    ...
    return mca_common_ucx_from_ompi_datatype(datatype);
}
...
static void mca_common_ucx_datatype_ctx_construct(mca_common_ucx_datatype_ctx_t *ctx)
{
    ...
    ret = ompi_attr_create_keyval(TYPE_ATTR, copy_fn, del_fn,
                                  &ctx->datatype_attr_keyval, NULL, 0, NULL);
    if (ret != OMPI_SUCCESS) {
        return;
    }
