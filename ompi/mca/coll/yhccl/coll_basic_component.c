...
extern int pjt_yhccl_reduce_initialized;
extern int pjt_yhccl_alllreduce_initialized;

void pjt_yhccl_reduce_close(void);
...
static int yhccl_close(void)
{
    ...
    close_time++;
    if (pjt_yhccl_reduce_initialized)
    {
        pjt_yhccl_reduce_close();
    }
...
const mca_coll_base_component_3_0_0_t mca_coll_yhccl_component = {
