...
#include "./yhccl_allreduce_pjt/yhccl_reduce.h"
 ...
 *	yhccl_reduce_pjt
 *
 *	Function:	- reduce using other MPI collectives
 *	Accepts:	- same as MPI_Reduce()
 *	Returns:	- MPI_SUCCESS or error code
 */
static class pjtccl_contexts *pjt_yhccl_reduce_ctx = 0;
int pjt_yhccl_reduce_initialized = 0;

extern "C" void pjt_yhccl_reduce_close(void)
{
    pjt_yhccl_reduce_ctx->destroy();
}
...
int mca_coll_yhccl_pjt_reduce_global(const void *sbuf, void *rbuf, size_t count,
                                     ...
                                     mca_coll_base_module_t *module)
{
    if (pjt_yhccl_reduce_initialized == 0)
    {
        // pjt_tempp = new float[1 << 25];
        pjt_yhccl_reduce_ctx = new pjtccl_contexts();
        pjt_yhccl_reduce_ctx->init(MPI_COMM_WORLD);
        pjt_yhccl_reduce_initialized = 1;
        printf("YHCCL USED! (%i)\n", getpid());
    }
    ...
    yhccl_intra_node_reduce_pjt(sbuf, rbuf, count, dtype, op, root, comm);
