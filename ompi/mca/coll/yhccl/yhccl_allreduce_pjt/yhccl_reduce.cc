...
extern void pjt_reduce_from_innerf7(const void *datasend, void *datarecv, size_t count, int elem_sz, int root, MPI_Datatype mpitype, MPI_Op mpi_op, yhccl_op fp);
        ...
        // if(0)
        {
            ...
            for (ss = 0; ss < count; ss += ctx->intra_node_procn * step)
            {
                                ...
                                if (j == 0)
                                {
                                    ...
                                    if (ctx->intra_node_rank != intra_node_root)
                                    {
                                        if (ctx->_reduce_opt.using_non_temporal_memory_access == 0)
                                            ...
                                            pjt_source_cachebypass_memmove((void*)dest, source, countl * elem_sz);
                                        else
                                            pjt_memmove((void*)dest, source, countl * elem_sz);
                                    }
                                    else
                                    {
                                        memset((void*)dest, 0, countl * elem_sz);
                                    }
                                }
                                else
                                {
                                    ...
                                    if (ctx->intra_node_rank != intra_node_root)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
#ifdef PJT_MPI
                                                ompi_op_reduce(mpi_op, (void*)source, (void*)dest, countl, mpitype);
#endif
                                            }
                                        }
                                    }
                ...
                if (countl > 0)
                {
                    ...
                    {
                        source = (const void *)ctx->neigbbor_buffers[j * intra_numa_procn] + my_start * elem_sz;
                        dest = ctx->larger_msg_allreduce_result_start_0 + my_start * elem_sz;
    ...
    void hierarchy_reduce_scatter(const void *_datasend, int _count, int _elem_sz, MPI_Datatype _mpitype, MPI_Op _mpi_op, yhccl_op _fp, int _step, int _total_steps, int root)
    {
        ...
        if (ctx->_reduce_opt.intra_node_reduce_type == CacheEfficient)
        {
            ...
            if (ctx->_reduce_opt.using_non_temporal_memory_access == 1)
            {
                ...
                if (ctx->intra_node_rank == 0)
                {
                    pjt_source_cachebypass_memmove((void*)ctx->larger_msg_allreduce_result_start_0, datasend, count * elem_sz);
                }
                else
                {
            ...
            else
            {
                if (ctx->intra_node_rank == 0)
                {
                    pjt_memmove((void*)ctx->larger_msg_allreduce_result_start_0, datasend, count * elem_sz);
                }
                else
                {
                    pjt_memmove((void*)ctx->larger_msg_allreduce_my_sendbuf, datasend, count * elem_sz);
                }
            }
        }
        step = _step;
...
extern void yhccl_sum_float_op_A_plus_B_eq_C(const void *a, const void *b, void *c, int *len);
                ...
                {
                    {
                        for (int i = 0; i < inter_numa_procn; i++)
                        {
                            ...
                            {
                                source = (const void *)ctx->neigbbor_buffers[j + inter_numa_rank * intra_numa_procn] + my_start * elem_sz;
                                countl = std::min(count - my_start, step);
                                if (countl > 0)
                                {
                                    if (0 != fp)
                        ...
                        {
                            source = (const void *)ctx->neigbbor_buffers[i * intra_numa_procn] + my_start * elem_sz;
                            countl = std::min(count - my_start, step);
                            if (countl > 0)
                            {
                                if (0 != fp)
...
void pjt_reduce_from_innerf7_new(const void *datasend, void *datarecv, int count, int elem_sz, int root, MPI_Datatype mpitype, MPI_Op mpi_op, yhccl_op fp)
{
    ...
    // if (ctx->global_rank == root)
    {
        if (ctx->_reduce_opt.using_non_temporal_memory_access == 0)
        {
            pjt_memmove(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, size);
        }
        else if (ctx->_reduce_opt.using_non_temporal_memory_access == 1)
        {
            pjt_target_cachebypass_memmove(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, size);
        }
        else
        {
            memmove(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, size);
        }
    }
    // MPI_Barrier(MPI_COMM_WORLD);
}
    ...
    MPI_Comm communicator)
{
    ...
    pjt_reduce_from_innerf7_new(send_data, recv_data, (int)count, elem_sz, root, datatype, op, reduce_op);
    return MPI_SUCCESS;
}