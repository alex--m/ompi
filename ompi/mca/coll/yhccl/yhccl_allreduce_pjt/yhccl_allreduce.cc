...
void Reduce_intra_node(const void *sendbuf, int count, int elem_sz, yhccl_op op, int *counts, int *starts)
{
    ...
    for (int ss = 0; ss < count; ss += slice_count * ctx->intra_node_procn)
    {
        for (int i = 0; i < ctx->intra_node_procn; i++)
        {
            ...
            if (countl > 0)
            {
                void *dest = (char*)ctx->larger_msg_allreduce_result_start_0 + sliceStart * elem_sz;
                const void *source = (char*)sendbuf + sliceStart * elem_sz;
    ...
    if (0)
        for (int i = 0; i < ctx->intra_node_procn; i++)
        {
            ...
            void *dest = (char*)ctx->larger_msg_allreduce_result_start_0 + starts[slice_id] * elem_sz;
            void *source = (char*)sendbuf + starts[slice_id] * elem_sz;
    ...
    int reqn = 0;
    {
        ...
        for (int ss = 0; ss < count; ss += ctx->intra_node_procn * step)
        {
            ...
            for (int i = 0; i < ctx->intra_node_procn; i++)
            {
                ...
                if (countl > 0)
                {
                    dest = (char*)ctx->larger_msg_allreduce_result_start_0 + sliceStart * elem_sz;
                    void *source = (char*)datasend + sliceStart * elem_sz;
                    while (ctx->allreduce_flags[slice_start + slice_lid] != i)
                        ;
                    if (i == 0)
                    {
...
void Reduce_intra_node_onSHM(int count, int elem_sz, yhccl_op op, int *counts, int *starts)
{
    ...
    for (int ss = 0; ss < count; ss += slice_ct)
    {
        if (sliceid % ctx->intra_node_procn == ctx->intra_node_rank)
        {
            ...
            void *dest = (char*)ctx->larger_msg_allreduce_result_start_0 + ss * elem_sz;
            for (int i = 0; i < ctx->intra_node_procn; i++)
            {
                void *source = (char*)ctx->neigbbor_buffers[i] + ss * elem_sz;
                // &(sendbuf[starts[slice_id]]);
                if (i == 0)
                {
                    memcpy(dest, source, countl * elem_sz);
    ...
    for (int ss = 0; ss < count; ss += slice_ct)
    {
        if (sliceid % ctx->intra_node_procn == ctx->intra_node_rank)
        {
            ...
            for (int hh = ss; hh < ss + countl; hh += slice_slice_count)
            {
                ...
                void *dest = (char*)ctx->larger_msg_allreduce_result_start_0 + hh * elem_sz;
                for (int i = 0; i < ctx->intra_node_procn; i++)
                {
                    void *source = (char*)ctx->neigbbor_buffers[i] + hh * elem_sz;
                    {
                        if (i == 0)
            ...
            if (ctx->inter_node_procn > 1 && countl > 0)
            {
                float *p = (float *)(ctx->larger_msg_allreduce_result_start_0 + ss * elem_sz);
...
void intel_rg_all_reduce(const void *datasend, void *datarecv, int count, int elem_sz, MPI_Op mpi_op, yhccl_op reducefp = 0)
{
    ...
    for (int h = 0; h < total_steps + tree_hight + 1; h++)
    {
        ...
        if (s >= 0 && s < total_steps)
        {
            ...
            {
                memmove((char*)ctx->neigbbor_buffers[ctx->intra_node_rank] + (s& 0x1)*step * elem_sz, datasend + s * step * elem_sz, ct * elem_sz);
            }
    ...
    MPI_Barrier(ctx->Comm_intra_node);
    // if (ctx->intra_node_rank == 0)
        memmove(datarecv, (const void*)ctx->neigbbor_buffers[0], count * elem_sz);
}
...
void Rabenseifner_reduce_scatter(const void *datasend, void *datarecv, size_t count, int elem_sz, MPI_Op mpi_op, yhccl_op reducefp = 0)
{
    //整个过程分威reduce和bcast部分。
    yhccl_contexts *ctx = yhccl_contexts::_ctx;
    ...
    for (int i = 0; i < steps; i++)
    {
        ...
        if(i == 0)
        {
            sendbuffer = (char*)datasend + send_startg * elem_sz;
        }else{
            sendbuffer = (char*)bufreduction + send_startg * elem_sz;
        }
        void *recvbuffer =  (char*)buftmp + recv_startg * elem_sz;
        ...
        {
            if(i == 0)
                float_sum_abc((const float *)((char*)datasend + recv_startg * elem_sz), (float*)recvbuffer, (float*)((char*)bufreduction + recv_startg * elem_sz), step);
            else{
                float_sum_abc((const float *)((char*)bufreduction + recv_startg * elem_sz), (float*)recvbuffer, (float*)((char*)bufreduction + recv_startg * elem_sz), step);
            }
        }
        // if(i == steps-1)
        // {
...
{
    yhccl_contexts *ctx = yhccl_contexts::_ctx;
    void *recvbuf = (void*)ctx->larger_msg_allreduce_result_start_1;
...
void *M_leader_reduce_scatter_ring(void *sendbuf, int start, int elem_sz, size_t count, int *counts, int *starts, int procn, int rank, MPI_Comm comm, yhccl_op reduce_op)
{
    // reduce_scatter，将消息分为多份
    int slice_c = count / procn;
    int remain = count % procn;
...
void yhccl_allreduce(const void *datasend, void *datarecv, size_t count, MPI_Datatype mpitype, MPI_Op mpi_op, yhccl_op reducefp = 0)
{
...
// #endif
    {
        ...
        case DPML:
        {
            ...
            if (ctx->intra_node_procn > 1)
            {
                ...
                case CacheEfficient:
                    ...
                    Reduce_intra_node_onSHM((int)count, elem_sz, reduce_op, counts_intra_node, starts_intra_node);
                    break;
                case MemoryEfficient:
                    // puts("516");
                    Reduce_intra_node(datasend, (int)count, elem_sz, reduce_op, counts_intra_node, starts_intra_node);
                    MPI_Barrier(ctx->Comm_intra_node);
                    break;
                }
            }
            else
                memcpy((void*)ctx->larger_msg_allreduce_result_start_0, datasend, count * elem_sz);
            void *sendb = (void*)ctx->larger_msg_allreduce_result_start_0 + starts_intra_node[ctx->intra_node_rank] * elem_sz;
            ...
            case intra_node_bcast::MEMCPY:
                memcpy(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, count * elem_sz);
                break;
            case intra_node_bcast::CacheEfficientBcast:
                Bcast_intra_node_memory_efficient((void*)ctx->larger_msg_allreduce_result_start_0, (int)count, elem_sz, datarecv);
                break;
            default:
                break;
            }
        ...
        case PIPELINED_DPML:
        {
            ...
            case CacheEfficient:
                ...
                MPI_Barrier(ctx->Comm_intra_node);
                pipelined_dpml_cache_efficient((int)count, elem_sz, reduce_op, counts_intra_node, starts_intra_node, mpitype, mpi_op);
                MPI_Barrier(ctx->Comm_intra_node);
                break;
            case MemoryEfficient:
                if (ctx->intra_node_rank == 0)
                {
                    ...
                    memset((void*)ctx->allreduce_flags, 0, ct * sizeof(unsigned long long));
                }
                MPI_Barrier(ctx->Comm_intra_node);
                pipelined_dpml_memory_efficient(datasend, (int)count, elem_sz, reduce_op, counts_intra_node, starts_intra_node, mpitype, mpi_op);
                MPI_Barrier(ctx->Comm_intra_node);
                if (ctx->intra_node_rank == 0)
                {
                    //清理所有内存标志。
        ...
        case INTEL_RG:
        {
            intel_rg_all_reduce(datasend, datarecv, (int)count, elem_sz, mpi_op, reduce_op);
            break;
        }
        case RING_AR:
        {
            pjt_ring_all_reduce(datasend, datarecv, (int)count, elem_sz, mpi_op, reduce_op);
            break;
        }
        case R_ALL_REDUCE:
        {
