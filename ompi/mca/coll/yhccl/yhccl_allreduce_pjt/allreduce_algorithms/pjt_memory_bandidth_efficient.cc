...
void *sendb, *recvb;
void reduce_scatter_pipeline(void *sendbuf, void *recvbuf, size_t count, int elemsz, yhccl_op op, int *counts, int *starts)
{
}
void broadcast(void *sendbuf, void *recvbuf, size_t count, int elemsz, int *counts, int *starts)
{
}

struct intra_node_reduce
{
	intra_node_reduce(void *datasend, void *datarecv, size_t count, int elem_sz, yhccl_op fp)
	{
		this->sendb = datasend;
		this->recvb = datarecv;
		this->count = count;
	...
	void proc()
	{
		...
		if (ctx->intra_node_rank == 0)
		{
			...
			void *endaddr = (void*)ctx->larger_msg_allreduce_result_start_0 + ss * elem_sz;
		...
		else
		{
			...
			void *dest = (void*)ctx->larger_msg_allreduce_result_start_0 + ss * elem_sz;
			// __sync_synchronize();
			if (ctx->_opt.open_intra_node_communication == 1)
...
{
	intra_node_broadcast(void *datasend, void *datarecv, size_t count, int elem_sz, yhccl_op fp)
	{
		this->sendb = datasend;
		this->recvb = datarecv;
		this->count = count;
	...
	void proc()
	{
		...
		//     puts("bcast");
		int local_ct = std::min(count - ss, step);
		void *source = (void*)ctx->larger_msg_allreduce_result_start_0 + ss * elem_sz;
...
{
	inter_node_allreduce1(void *datasend, void *datarecv, size_t count, int elem_sz, yhccl_op fp)
	{
		this->sendb = datasend;
		this->recvb = datarecv;
		this->count = count;
	...
	void proc()
	{
		...
		void *addrs = (void*)ctx->larger_msg_allreduce_result_start_0 + ss * elem_sz;
		float *p = (float*)addrs;
		if (ctx->inter_node_procn > 1)
			if (ctx->intra_node_procn == 1)
			{
...
void pjt_memory_bandwidth_efficient_allreduce_callback(int thid, int sig)
{
	...
	for (int start = thid * slice_sz; start < count; start += slice_sz * thread_ct)
	{
		...
		void *addrs = (void*)ctx->larger_msg_allreduce_result_start_0 + start * elem_sz;
		float *p = (float*)addrs;
...
// Generator<int> ring_allgather(void *sendbuf, size_t count, int elem_sz, MPI_Comm comm, int rank, int procn, int *counts, int *starts, int flag)
// inline void ring_allgather(void *sendbuf, size_t count, int elem_sz, MPI_Comm comm, int rank, int procn, int *counts, int *starts, int flag)
...
{
	Generator<int> ring_reduce_scatter_inplace(void *sendbuf, size_t count,
											   int elem_sz, MPI_Comm comm, int rank,
											   int procn, int *counts, int *starts,
											   yhccl_op fp, int flag);
	Generator<int> iallreduce_inplace(void *sendbuf, size_t count, int elem_sz, yhccl_op fp, int reqn)
	{
...
inline Generator<int> PJT_Iallreducer::ring_reduce_scatter_inplace(void *sendbuf, size_t count, int elem_sz, MPI_Comm comm, int rank, int procn, int *counts, int *starts, yhccl_op fp, int flag)
{
...
#ifdef CPP20_COROUTINE
void innerf(void *datasend, void *datarecv, size_t count, int elem_sz, yhccl_op fp)
{
	// puts("83");
	sendb = datasend;
	recvb = datarecv;
	...
	if (0)
	{
		...
		int reqn = 0;
		{
			...
			for (int ss = 0; ss < count; ss += ctx->intra_node_procn * step)
			{
				...
				for (int i = 0; i < ctx->intra_node_procn; i++)
				{
					...
					if (countl > 0)
					{
						dest = (void*)ctx->larger_msg_allreduce_result_start_0 + sliceStart * elem_sz;
		...
		MPI_Barrier(ctx->Comm_global);
		{
			...
			for (int s = 0; s < total_steps; s += wait_slice_count)
			{
				...
				void *source = (void*)ctx->larger_msg_allreduce_result_start_0 + s * step * elem_sz;
				...
				if (prev_sliceid_start >= 0)
				{
					...
					for (int i = 0; i < ctx->intra_node_procn; i++)
					{
						...
						while (ctx->allreduce_flags[slice_id] != ctx->intra_node_procn + 1)
							;
						void *start_addr = (void*)ctx->larger_msg_allreduce_result_start_0 + start * elem_sz;
					...
					void *start_addr = (void*)ctx->larger_msg_allreduce_result_start_0 + prev_sliceid_start * step * elem_sz;
			...
			int prev_sliceid_start = sliceid_start - ctx->intra_node_procn;
			{
				...
				for (int i = 0; i < wait_block_count; i++)
				{
					...
					while (ctx->allreduce_flags[slice_id] != ctx->intra_node_procn + 1)
						;
					// __sync_synchronize();
					volatile void *start_addr = (void*)ctx->larger_msg_allreduce_result_start_0 + start * elem_sz;
					// void *end_addr = datarecv + start * elem_sz;
					// memmove(end_addr, start_addr, ct * elem_sz);
					// for (int i = 0; i < ct; i++)
					// {
				...
				int ct = std::min(count - prev_sliceid_start * step, step * ctx->intra_node_procn);
				volatile void *start_addr = (void*)ctx->larger_msg_allreduce_result_start_0 + prev_sliceid_start * step * elem_sz;
				void *end_addr = datarecv + prev_sliceid_start * step * elem_sz;
				// memmove(end_addr, start_addr, ct * elem_sz);
...
#ifdef CPP20_COROUTINE
void innerf1(void *datasend, void *datarecv, size_t count, int elem_sz, yhccl_op fp)
{
...
struct pjt_allreducer_pt
{
	...
	int ring_reduce_scatter_inplace_push()
	{
		PT_BEGIN(&RRS_pt);
		for (RRS_i = 1; RRS_i < RRS_procn; RRS_i++)
		{
			...
			void *sendbuf = RRS_sendbuf + elem_sz * (send_slice_id * RRS_step +
			...
			MPI_Irecv((void*)ring_rs_recvbuf, RRS_recvct * elem_sz, MPI_CHAR, ctx->inter_node_rank + RRS_dim * (ring_rs_recvsource - RRS_rank), reqn, ctx->Comm_inter_node, &req_recv);
...
#ifdef PJT_MPI
			ompi_op_reduce(pjt_mpiop, (void*)ring_rs_recvbuf, RRS_dest, RRS_recvct, pjt_mpitype);
#else
	...
	int RRS_step;
	int RRS_remain;
	void ring_reduce_scatter_inplace(void *sendbuf, size_t count, int dim, int rank, int procn)
	{
	...
	void tree_reduce_bcast(void *sendbuf, size_t count, int dim, int rank, int procn)
	{
		_TR_pt.lc = NULL;
		_TR_k = 4;
		_TR_sendbuf = sendbuf;
	...
	void *RA_recvbuf;
	int RA_i;
	void iphring_allgather(void *sendb, size_t count, int dim, int mrank, int procn)
	{
...
public:
	...
	char push_barrier()
	{
		...
		else
		{
			...
			{
				barrier_p = (volatile char *)ctx->intra_node_flags[ctx->intra_node_rank];
				*barrier_p = 'S';
				memory_fence();
				while (*barrier_p != 'R')
					PT_YIELD(&barrier_pt);
	...
	void multi_leader_hierarchy_allreduce(int leadern, int slicesz, int total_slicen, void *sendbuf, size_t count, int elem_sz, MPI_Op mpi_op, yhccl_op fp, MPI_Datatype mpitype)
	{
...
void innerf4(const void *datasend, void *datarecv, size_t count, int elem_sz, MPI_Datatype mpitype, MPI_Op mpi_op, yhccl_op fp)
{
	// puts(" innerf4");
	pjt_mpitype = mpitype;
	pjt_mpiop = mpi_op;
	...
	if (ctx->intra_node_rank == 0)
	{
		...
		memset((void*)ctx->allreduce_flags, -1, ct * sizeof(unsigned long long));
	}
	MPI_Barrier(ctx->Comm_intra_node);
	pjt_inter_node_allreduce1 pjt_iallreduce;
	pjt_iallreduce.multi_leader_hierarchy_allreduce(leadern, step, total_steps, (void*)ctx->larger_msg_allreduce_result_start_0, count, elem_sz, mpi_op, fp, mpitype);

	{
		...
		void *recvbuf = (void*)ctx->larger_msg_allreduce_result_start_0 + s * elem_sz * ctx->intra_node_rank;
		// MPI_Reduce_scatter(datasend, recvbuf, counts, mpitype, mpi_op, ctx->Comm_intra_node);
		MPI_Reduce(datasend, (void*)ctx->larger_msg_allreduce_result_start_0, count, mpitype, mpi_op, 0, ctx->Comm_intra_node);
	}
	// MPI_Barrier(ctx->Comm_intra_node);
	if (ctx->intra_node_rank == 0)
	{
	...
	MPI_Barrier(ctx->Comm_intra_node);
	memmove(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, count * elem_sz);
}

void innerf5(const void *datasend, void *datarecv, size_t count, int elem_sz, MPI_Datatype mpitype, MPI_Op mpi_op, yhccl_op fp)
{
	pjt_mpitype = mpitype;
	pjt_mpiop = mpi_op;
	yhccl_contexts *ctx = yhccl_contexts::_ctx;
		...
		{
			{
				...
				for (s = 0; s < total_steps; s += ctx->intra_node_procn)
				{
					...
					void *start_addr = (void*)ctx->larger_msg_allreduce_result_start_0 + s * step * elem_sz;
...
#ifdef PJT_AVX_ASSEMBLY_MEMCPY
				if (ctx->_opt.using_non_temporal == 1)
					pjt_target_cachebypass_memmove(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, count * elem_sz);
				else
					pjt_memmove(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, count * elem_sz);
#else
				memmove(datarecv, (void*)ctx->larger_msg_allreduce_result_start_0, count * elem_sz);
#endif
			}
...
// 							dest = (void*)ctx->larger_msg_allreduce_result_start_0 + my_start * elem_sz;
// 							// printf("original %d %f\n", ctx->intra_node_rank, *(float *)dest);
// 							countl = std::min(count - my_start, step);
// 							for (j = 1; j < ctx->intra_node_procn; j++)
// 							{
// 								// 遍历intra_numa_procn
// 								source = (void*)ctx->neigbbor_buffers[j] + my_start * elem_sz;

// 								// source = (void*)ctx->neigbbor_buffers[ctx->intra_node_rank] + my_start * elem_sz;
// 								if (countl > 0)
// 								{
// 									if (0 != fp)
// 									{
						...
						//第一步socket内，第二步socket-间
						{
							for(int i = 0;i<inter_numa_procn;i++)
							{
								...
								if (ctx->intra_node_rank < intra_numa_procn)
									...
									dest = (void*)ctx->neigbbor_buffers[inter_numa_rank * intra_numa_procn] + my_start * elem_sz;
								for(int j = 1;j<intra_numa_procn;j++)
								{
									source = (void*)ctx->neigbbor_buffers[j + inter_numa_rank * intra_numa_procn] + my_start * elem_sz;
									countl = std::min(count - my_start, step);
									if (countl > 0)
									{
										if (0 != fp)
										{
											fp(source, (void*)dest, &countl, 0);
										}
										else
										{
#ifdef PJT_MPI
											ompi_op_reduce(mpi_op, source, (void*)dest, countl, mpitype);
#endif
										}
									}
								}
							...
							{
								source = (void*)ctx->neigbbor_buffers[i * intra_numa_procn] + my_start * elem_sz;
								countl = std::min(count - my_start, step);
								if (countl > 0)
								{
									if (0 != fp)
							...
							for (j = 0; j < intra_numa_procn; j++)
							{
								...
								if (countl > 0)
								{
									...
									if (ctx->intra_node_rank < intra_numa_procn)
											...
											dest = (void*)ctx->neigbbor_buffers[inter_numa_rank * intra_numa_procn] + (my_start + (loopi &0x1) * ctx->intra_node_procn * step) * elem_sz;
									...
									if (j == 0)
									{
										...
										if (ctx->_opt.using_non_temporal <= 1)
										{
											pjt_memmove((void*)dest, source, countl * elem_sz);
										}
										else if (ctx->_opt.using_non_temporal == 2)
										{
											memory_fence();
											pjt_target_cachebypass_memmove((void*)dest, source, countl * elem_sz);
											memory_fence();
										}else{
											memmove((void*)dest, source, countl * elem_sz);
										}
#else
										memmove((void*)dest, source, countl * elem_sz);
#endif
...
// 						// 接下来在NUMA间进行规约
						...
						if (countl > 0)
						{
							...
							{
								source = (void*)ctx->neigbbor_buffers[j * intra_numa_procn] + (my_start + (loopi &0x1)*ctx->intra_node_procn *step) * elem_sz;
	...
	// MPI_Barrier(MPI_COMM_WORLD);
		...
		step = _step;
		total_steps = _total_steps;
		dest_buf = (void*)ctx->larger_msg_allreduce_result_start_0;
		if (ctx->_opt.intra_node_reduce_type == CacheEfficient)
		{
// #ifdef PJT_AVX_ASSEMBLY_MEMCPY
			// if (intra_numa_rank != 0)
				if (ctx->intra_node_rank == 0)
				{
					pjt_source_cachebypass_memmove((void*)dest_buf, datasend, count * elem_sz);
				}
				else
				{
					pjt_source_cachebypass_memmove((void*)ctx->larger_msg_allreduce_my_sendbuf, datasend, count * elem_sz);
				}
// #else
// #endif
		}
		...
		{
			{
				...
				for (s = 0; s < total_steps; s += ctx->intra_node_procn)
				{
					...
					if (ctx->_opt.pjt_inner_cpy == 1)
					{
						...
						void *start_addr;
						// if (ctx->inter_node_procn == 1)
						// 	start_addr = (void*)ctx->larger_msg_allreduce_result_start_0 ;
						// else
							start_addr = (void*)ctx->larger_msg_allreduce_result_start_0 + s * step * elem_sz;
						void *end_addr = datarecv + s * step * elem_sz;
						int ct = std::min(count - s * step, step * ctx->intra_node_procn);
...
void innerf7(const void *datasend, void *datarecv, int count, int elem_sz, MPI_Datatype mpitype, MPI_Op mpi_op, yhccl_op fp)
{
...

	{
		...
		// if(0)
		{
			...
			MPI_Barrier(ctx->Comm_intra_node);
			const void * sourceb = (const void*)RS.dest_buf;
		...
		if (count * elem_sz <= 256*1024)
			{
					...
					if (ctx->intra_node_rank == ctx->intra_node_procn / 2)
						memmove((void*)sourceb1, sourceb, count * elem_sz);
					MPI_Barrier(ctx->Comm_intra_node);
					if (ctx->intra_node_rank >= ctx->intra_node_procn / 2)
						sourceb = sourceb1;
			}
...
int pjt_memory_bandwidth_efficient_allreduce(const void *datasend, void *datarecv, size_t count, int elem_sz, MPI_Datatype mpitype, MPI_Op mpi_op, yhccl_op fp)
{
	...
	{
		// fflush(stdou1t);
		innerf7(datasend, datarecv, (int)count, elem_sz, mpitype, mpi_op, fp);
	}
	return 0;
}
