...
extern "C" int yhccl_intra_node_bcast_pjt(void *buffer, size_t count, MPI_Datatype datatype, int root, MPI_Comm comm)
{
            ...
            if (i < sz)
            {
                ...
                if (ctx->global_rank == root)
                {
                    ...
                    if (ctx->_bcast_opt.using_non_temporal_memory_access == 0)
                        pjt_memmove((void *)socket_buffers[send_index & 0x1], (const void *)buffer + i, lsz);
                    else if (ctx->_bcast_opt.using_non_temporal_memory_access == 1)
                    {
                        bcast_copy((void *)socket_buffers[send_index & 0x1], (const void *)buffer + i, lsz, W, C, 0);
                    }
                    else
                    {
                        pjt_target_cachebypass_memmove((void *)socket_buffers[send_index & 0x1], (const void *)buffer + i, lsz);
                    }
                    send_index++;
                }
            }
            if (i > 0)
            {
                ...
                if (lsz > 0)
                    if (ctx->global_rank != root)
                    {
                        ...
                        if (ctx->_bcast_opt.using_non_temporal_memory_access == 0)
                            pjt_memmove((void *)buf_recv, (const void *)buf_tmp, lsz);
                        else if (ctx->_bcast_opt.using_non_temporal_memory_access == 1)
                            bcast_copy((void *)buf_recv, (const void *)buf_tmp, lsz,W,C,1);
                    // pjt_target_cachebypass_memmove(buf_recv, buf_tmp, lsz);
                        else
                            pjt_target_cachebypass_memmove((void *)buf_recv, (const void *)buf_tmp, lsz);
    ...
    else
    {
        ...
        for (int i = 0; i < sz; i += slice_sz)
        {
            ...
            if (ctx->global_rank == root)
            {
                // root
                if (ctx->_bcast_opt.using_non_temporal_memory_access == 0)
                    pjt_memmove((void *)shm_buf, (const void *)buffer + i, lsz); // temporal mov to nt
                else
                {
                    pjt_source_cachebypass_memmove((void *)shm_buf, (const void *)buffer + i, lsz);
                }
            ...
            else
            {
                yhccl_barrier_intra_node();
                if (ctx->_bcast_opt.using_non_temporal_memory_access == 0)
                    pjt_memmove((void *)buffer + i, (const void *)shm_buf, lsz); // nt mov to tmporal
                else
                    pjt_target_cachebypass_memmove((void *)buffer + i, (const void *)shm_buf, lsz);
                // child
            }
        }
    }
