...
void yhccl_contexts::init_large_msg_allreduce_buffer(int intra_node_rank, int intra_procn, int inter_node_rank)
{
    ...
    MPI_Barrier(Comm_intra_node);
    //分配节点内同步flag
    intra_node_flags = new volatile void *[intra_node_procn];
    volatile void *p = larger_msg_allreduce_result_start_1 + large_msg_allreduce_sendbuff_sz;
    for (int i = 0; i < intra_node_procn; i++)
    {
    ...
    if (intra_node_rank == 0)
    {
        ...
        memset((void*)larger_msg_allreduce_result_start_0, 0, large_msg_allreduce_buff_sz);
        memset((void*)larger_msg_allreduce_result_start_1, 0, large_msg_allreduce_buff_sz);
    }
    ...
    memset((void*)allreduce_flags, 0, sizeof(unsigned long long) * (1UL << 22));
...
void yhccl_sum_float_op_A_plus_B_eq_C(const void *a, const void *b, void *c, int *len)
{
    // puts("410")
    // memory_fence();
    // dest和source必须按128位/16 Byte对齐
