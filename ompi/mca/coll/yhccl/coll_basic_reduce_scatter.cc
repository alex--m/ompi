 ...
 */
int
mca_coll_yhccl_reduce_scatter_intra(const void *sbuf, void *rbuf,
                                    ompi_count_array_t rcounts_arr,
                                    struct ompi_datatype_t *dtype,
                                    struct ompi_op_t *op,
                                    struct ompi_communicator_t *comm,
                                    mca_coll_base_module_t *module)
{
    ...
    OMPI_DISP_ARRAY_INIT(&disps_arr, disps);

    disps[0] = 0;
    for (i = 0; i < (size - 1); ++i) {
        disps[i + 1] = disps[i] + ompi_count_array_get(rcounts_arr, i);
    }
    count = disps[size - 1] + ompi_count_array_get(rcounts_arr, size - 1);
    ...
    if ((op->o_flags & OMPI_OP_FLAGS_COMMUTE) &&
        ...
        if (tmp_rank >= 0) {
            ...
            for (i = 0 ; i < tmp_size ; ++i) {
                if (i < remain) {
                    ...
                    tmp_rcounts[i] = ompi_count_array_get(rcounts_arr, i * 2 + 1) +
                                     ompi_count_array_get(rcounts_arr, i * 2);
                } else {
                    tmp_rcounts[i] = ompi_count_array_get(rcounts_arr, i + remain);
                }
            }
            ...
            if (0 != ompi_count_array_get(rcounts_arr, rank)) {
                err = ompi_datatype_sndrcv(result_buf + disps[rank] * extent,
                                           ompi_count_array_get(rcounts_arr, rank),
                                           dtype, rbuf,
                                           ompi_count_array_get(rcounts_arr, rank),
                                           dtype);
                if (OMPI_SUCCESS != err) {
        ...
        if (rank < 2 * remain) {
            if ((rank & 1) == 0) {
                if (ompi_count_array_get(rcounts_arr, rank)) {
                    err = MCA_PML_CALL(recv(rbuf, ompi_count_array_get(rcounts_arr, rank),
                                            dtype, rank + 1,
                                            MCA_COLL_BASE_TAG_REDUCE_SCATTER,
                                            comm, MPI_STATUS_IGNORE));
                    if (OMPI_SUCCESS != err) goto cleanup;
                ...
                if (ompi_count_array_get(rcounts_arr, rank - 1)) {
                    err = MCA_PML_CALL(send(result_buf + disps[rank - 1] * extent,
                                            ompi_count_array_get(rcounts_arr, rank - 1),
                                            dtype, rank - 1,
                                            MCA_COLL_BASE_TAG_REDUCE_SCATTER,
                                            MCA_PML_BASE_SEND_STANDARD,
                                            comm));
        ...
        /* scatter */
        if (MPI_SUCCESS == err) {
            err = comm->c_coll->coll_scatterv(recv_buf, rcounts_arr, disps_arr, dtype,
                                              rbuf, ompi_count_array_get(rcounts_arr, rank), dtype, 0,
 ...
 */
int
mca_coll_yhccl_reduce_scatter_inter(const void *sbuf, void *rbuf,
                                    ompi_count_array_t rcounts_arr,
                                    struct ompi_datatype_t *dtype,
                                    struct ompi_op_t *op,
                                    struct ompi_communicator_t *comm,
                                    mca_coll_base_module_t *module)
{
    ...
    for (totalcounts = 0, i = 0; i < lsize; i++) {
        totalcounts += ompi_count_array_get(rcounts_arr, i);
    }
    ...
    err = comm->c_local_comm->c_coll->coll_scatterv(lbuf, rcounts_arr, disps_arr,
                                                    dtype, rbuf, ompi_count_array_get(rcounts_arr, rank),
                                                    dtype, 0, comm->c_local_comm,
