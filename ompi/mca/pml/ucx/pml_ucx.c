                             ...
                             struct ompi_request_t **request)
{
    ...
    req->datatype                 = mca_common_ucx_get_datatype(datatype);
...
static inline int
mca_pml_ucx_recv_complete(ompi_datatype_t *datatype, size_t count,
                          ucs_status_t status, ucp_tag_recv_info_t* info,
                          ompi_status_public_t* mpi_status)
{
    int result = mca_pml_ucx_set_recv_status_public(mpi_status, status, info);
                      ...
                      struct ompi_request_t **request)
{
    ...
    req = (ompi_request_t*)mca_pml_ucx_common_send_nbx(ep, buf, count, datatype,
                                                       ...
                                                       &mca_common_ucx_get_op_data(datatype)->op_param.isend);
#else
    ...
    if (req == NULL) {
        ...
        *request = &ompi_common_ucx.completed_request;
        ...
        req->req_mpi_object.comm = comm;
#if MPI_VERSION >= 4
        if (OPAL_LIKELY(mca_common_ucx_request_cancel_send == req->req_cancel)) {
            req->req_cancel      = mca_common_ucx_request_cancel_send;
        }
#endif
        *request                 = req;
        return OMPI_SUCCESS;
