...
int mca_scoll_ucx_init_query(bool enable_progress_threads, bool enable_mpi_threads)
{
    return OSHMEM_SUCCESS;
}

static void mca_scoll_ucx_module_clear(mca_scoll_ucx_module_t *mpi_module)
{
...
static void mca_scoll_ucx_module_construct(mca_scoll_ucx_module_t *mpi_module)
{
    mca_scoll_ucx_module_clear(mpi_module);
}

static void mca_scoll_ucx_module_destruct(mca_scoll_ucx_module_t *mpi_module)
{
...
static int mca_scoll_ucx_save_coll_handlers(mca_scoll_base_module_t *module, oshmem_group_t *osh_group)
{
    mca_scoll_ucx_module_t* mpi_module = (mca_scoll_ucx_module_t*) module;
 ...
 */
static int mca_scoll_ucx_module_enable(mca_scoll_base_module_t *module,
                                        oshmem_group_t *osh_group)
{

    if (OSHMEM_SUCCESS != mca_scoll_ucx_save_coll_handlers(module, osh_group)){
...
mca_scoll_ucx_comm_query(oshmem_group_t *osh_group, int *priority)
{
    mca_scoll_base_module_t *module;
    mca_scoll_ucx_module_t *mpi_module;
    int err, i;
    int tag;
    ompi_group_t* world_group, *new_group;
    ompi_communicator_t* newcomm = NULL;
    *priority = 0;
    mca_scoll_ucx_component_t *cm;
    cm = &mca_scoll_ucx_component;
    int* ranks;
    if (!cm->mpi_enable){
        return NULL;
    }
    ...
    mpi_module = OBJ_NEW(mca_scoll_ucx_module_t);
    if (!mpi_module){
        return NULL;
    }
    mpi_module->comm = osh_group->ompi_comm;

    mpi_module->super.scoll_module_enable = mca_scoll_ucx_module_enable;
    mpi_module->super.scoll_barrier = mca_scoll_ucx_barrier;
    mpi_module->super.scoll_broadcast = mca_scoll_ucx_broadcast;
    mpi_module->super.scoll_reduce = mca_scoll_ucx_reduce;
    mpi_module->super.scoll_collect = mca_scoll_ucx_collect;
...
OBJ_CLASS_INSTANCE(mca_scoll_ucx_module_t,
                   mca_scoll_base_module_t,
                   mca_scoll_ucx_module_construct,
                   mca_scoll_ucx_module_destruct);
